        ...
        fileUpload: function (options) {
            var para = {
                ...
                uploadComplete: function (res) {
					...
					res = eval('(' + res + ')');
					$('#preview').append('<input class="attachs" id="atx' + uploadCount + '" type="hidden" value="' + res.data.attach_id + '" />');
					$('#x' + uploadCount).attr('src', $('#imgcontentx' + uploadCount).val());
                    uploadCount++;
                    core.checkComplete();
                },
            ...
            var core = {
                ...
                previewImage: function (file) {
                    ...
                    img.file = file;
                    img.id='x' + uploadCount;
					  
                    $(para.addpic).before(img);
                    ...
                    reader.onload = (function (aImg) {
                        return function (e) {
                            ...
                            $(aImg).on("click", $(this), function () {
								if($(this).attr('src') == '/storage/app/wap/etwod/images/loading_1.gif'){//图片没有传完不能进行删除操作
									alert('图片还没传完，不能删除');
									return false;
								}
								$('#at' + $(this).attr('id')).remove();
                                $(this).remove();
								uploadCount--;
                            });
                        };
