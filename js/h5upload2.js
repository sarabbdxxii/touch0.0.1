        ...
        fileUpload: function (options) {
            var para = {
                ...
                uploadComplete: function (res) {
					...
					res = eval('(' + res + ')');
					$('#preview').append('<input class="attachs" id="atx' + imgCount + '" type="hidden" value="' + res.data.attach_id + '" />');
					$('#x' + imgCount).attr('src', $('#imgcontentx' + imgCount).val());
                    uploadCount++;
                    imgCount++
                    core.checkComplete();
                },
            ...
            var inputstr = '<span style="width:100%;height: 100%;display:block;z-index:99999;"><input id="fileImage"  style="opacity:0;width:100%;height:100%;position:absolute;top:0 ;left:0;" accept="image/jpeg,.jpg,image/gif,.gif,image/png,.png,image/bmp,.bmp,.jpeg"  type="file" size="30" name="fileselect[]" ' + multiple + '></span>';
            $(para.addpic).append(inputstr);
            ...
            var core = {
                    ...
                    for (var i = 0; i < count; i++) {
                        ...
                        if (item.size > 1024 * 1024 * 2) {
                            ...
                            (function(img) {
                                ...
                                mpImg.render(resImg, { maxWidth: 1000, maxHeight: 1000, quality: 1 }, function() {
                ...
                previewImage: function (file) {
                    ...
                    img.file = file;
                    img.id='x' + imgCount;
                    img_box.id='boxx' + imgCount;
                    img_close.id = 'del' + imgCount;


                    $(img_box).append(img);
                    $(img_box).append(img_close);
                    $(para.addpic).before(img_box);

                    var post_with = parseInt($(window).width())*0.3;
                    $("#preview img").width(post_with);
                    $("#preview img").height(post_with);
                    ...
                    $(img_close).attr("class","img_close")
                    // 使用FileReader方法显示图片内容
                    var reader = new FileReader();
                    reader.onload = (function (aImg) {
                        return function (e) {
                            //aImg.src = e.target.result;
							aImg.src = '/storage/app/wap/etwod/images/loading_1.gif';
							var imgcontent = $("<input type='hidden' id='imgcontent" + aImg.id + "' value='" + e.target.result + "'/>");
							$(".vote_abtn").after(imgcontent);
                            $(img_close).on("click", $(this), function () {
								if($(this).siblings('img').attr('src') == '/storage/app/wap/etwod/images/loading_1.gif'){
                                //图片没有传完不能进行删除操作
									alert('图片还没传完，不能删除');
									return false;
								}
                                $('#at' + $(this).siblings('img').attr('id')).remove();
                                $('#box' + $(this).siblings('img').attr('id')).remove();
								uploadCount--;
                            });
                        };
                    })(img);
            ...
            $(document).on("click", para.filebutton, function () {
                console.log(para.filebutton);
            });
            //console.log(para.uploadButton);
